...
usage(void)
{
	...
	,"  -H defines_file"
	,"  --defines=defines_file  write definitions to defines_file"
	,"  -i                      write interface (y.tab.i)"
	,"  -g                      write a graphical description"
	,"  -l                      suppress #line directives"
	,"  -L                      enable position processing, e.g., \"%locations\""
	,"  -o output_file"
	,"  --output=output_file    (default \"" OUTPUT_SUFFIX "\")"
...
getargs(int argc, char *argv[])
{
    ...
    /* Handle --defines=<defines_file> and --output=<output_file> for better
       bison compatibility. Instead of mucking around with getopt_long(), we
       translate them to -H<defines_file> and -o<output_file>. */
    for (i = 1; i < argc; ++i)
    {
	char *a = argv[i];
	if (strncmp (a, "--defines=", 10) == 0)
	{
	    a[8] = '-';
	    a[9] = 'H';
	    argv[i] = a + 8;
